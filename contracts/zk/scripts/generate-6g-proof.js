 ...
 *
 * Usage:
 *   node zk/scripts/generate-6g-proof.js
 */
...
async function generate6GProof() {
    ...
    // Circuit input format matching 6g_capability_proof.circom
    const input = {
    ...
    console.log('   b:', solidityProof.b.map(row => row.map(x => x.toString())));
    ...
    console.log('// Verify proof on-chain:');
    ...
    return { proof: solidityProof, publicInputs, publicSignals, isValid, capabilities, avgAccuracy };
}

async function main() {
    try {
        await generate6GProof();
        console.log('\n🎉 6G Proof generation completed successfully!');
    } catch (error) {
